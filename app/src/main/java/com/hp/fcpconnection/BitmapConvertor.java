 ...
 */
	...
	public byte[] convertBitmapToBuffer(Bitmap inputBitmap){
		...
		byte [] data = saveImage( mWidth, mHeight);
		return data;
    ...
    private byte[] saveImage(int width, int height)
	{
		byte[] array = null;
		ByteArrayOutputStream baos = new ByteArrayOutputStream();
		try {
			BMPFile bmpFile = new BMPFile();
			bmpFile.saveBitmap(baos, mRawBitmapData, width, height);
